...
df=pd.read_csv("data/삼성전자(주).csv")
# 결측치 확인
print(df.isnull().sum())
...
with open('model/삼성indextocoef.pkl','wb') as fp:
    pickle.dump(indextocoef, fp)
    print('dictionary saved successfully to file')

#2)save dictionary : texttoindex
with open('model/삼성texttoindex.pkl','wb') as fp:
    pickle.dump(texttoindex, fp)
    print('dictionary saved successfully to file')


#3)Logistic Regression save
filename = 'model/삼성lregression.rl'
pickle.dump(lr, open(filename,'wb'))



#modle read
with open('model/삼성indextocoef.pkl', 'rb') as fp:
    indextocoef = pickle.load(fp)
indextocoef

with open('model/삼성texttoindex.pkl', 'rb') as fp:
    texttoindex = pickle.load(fp)
texttoindex

lr = pickle.load(open('model/삼성lregression.rl', 'rb'))
lr.coef_[0]
